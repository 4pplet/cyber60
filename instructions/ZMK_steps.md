...
- The keymap files can be found in `zmk/app/boards/4pplet/cyber60/` (e.g. `cyber60_rev_e.keymap`)
I'd recommend to use Visual Studio Code for this, but use an editor of your liking.
## 4: Install ZMK toolchain
Follow the instructions by ZMK to install the toolchain, I highly recommend to use the docker setup.
https://zmk.dev/docs/development/setup
## 5: Build ZMK
In the terminal, navigate to the ZMK directory that was cloned from GitHub in step 2.
Change folder to app:
```bash
cd app
```

Build ZMK, replace the revision letter depending on what revision you are building:
```bash
west build -p -b cyber60@rev_e
```

Available revisions: `rev_a`, `rev_b`, `rev_c`, `rev_d`, `rev_e`

## 6: Locate the built .uf2
The file will be called `zmk.uf2` and located in `zmk/app/build/zephyr/`
